...
def ogrenci_ders_ekle_gui(cursor):
    def ekle():
        ogrenci_ad = ogrenci_entry.get()
        ders_ad = ders_entry.get()

        cursor.execute('SELECT id FROM ogrenci WHERE ad = ?', (ogrenci_ad,))
        ogrenci = cursor.fetchone()
        cursor.execute('SELECT id FROM ders WHERE ad = ?', (ders_ad,))
        ders = cursor.fetchone()

        if ogrenci and ders:
            veritabani_ogrenci_ders_ekle(cursor, ogrenci[0], ders[0])
            messagebox.showinfo("Başarılı", f"{ogrenci_ad} adlı öğrenci {ders_ad} adlı derse başarıyla eklendi.")
            ekle_penceresi.destroy()
        else:
            messagebox.showerror("Hata", "Öğrenci veya ders bulunamadı.")

    ekle_penceresi = tk.Toplevel()
    ekle_penceresi.title("Öğrenciye Ders Ekle")
    tk.Label(ekle_penceresi, text="Öğrenci Adı:").grid(row=0, column=0)
    ogrenci_entry = tk.Entry(ekle_penceresi)
    ogrenci_entry.grid(row=0, column=1)
    tk.Label(ekle_penceresi, text="Ders Adı:").grid(row=1, column=0)
    ders_entry = tk.Entry(ekle_penceresi)
    ders_entry.grid(row=1, column=1)
    tk.Button(ekle_penceresi, text="Ekle", command=ekle).grid(row=2, column=0, columnspan=2)
...
def main():
    ...
    tk.Button(root, text="Öğrenciye Ders Ekle", command=lambda: ogrenci_ders_ekle_gui(cursor)).pack(pady=5)
